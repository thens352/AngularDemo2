...
public class NcModemEndpoint {
	...
	public Response searchBarkod(@PathParam("barkod") Integer barkod) {
		NcModem entity = dao.searchRest(barkod, null);
		if (entity != null)
	...
	public Response searchSeriNo(@PathParam("cihazSeriNo") String cihazSeriNo) {
		NcModem entity = dao.searchRest(0, cihazSeriNo);
		if (entity != null)
			return Response.ok(entity).build();
		return Response.noContent().build();
	}
	...
	@Produces("application/json")
	public NcModem create(NcModem entity) {
	...
	@GET
	@Produces("application/json")
	public List<NcModem> listAll() {
		List<NcModem> results = dao.findAll();
		return results;
	}
	...
	@Path("/{id:[0-9][0-9]*}")
	@Consumes("application/json")
	public Response update(@PathParam("id") int id, NcModem entity) {
			if (entity == null) {
