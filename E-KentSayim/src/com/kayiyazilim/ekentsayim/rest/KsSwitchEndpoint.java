...
@Stateless
@Path("/ksswitch")
public class KsSwitchEndpoint {
	@EJB
	private KsSwitchDao dao;
	@EJB
	...
	public Response searchBarkod(@PathParam("barkod") Integer barkod) {
		KsSwitch entity = dao.searchRest(barkod, null);
		if (entity != null)
	...
	public Response searchSeriNo(@PathParam("cihazSeriNo") String cihazSeriNo) {
		KsSwitch entity = dao.searchRest(0, cihazSeriNo);
		if (entity != null)
			return Response.ok(entity).build();
		return Response.noContent().build();
	}
	...
	@Produces("application/json")
	public KsSwitch create(KsSwitch entity) {
	...
	@GET
	@Produces("application/json")
	public List<KsSwitch> listAll() {
		List<KsSwitch> results = dao.findAll();
		return results;
	}
	...
	@Path("/{id:[0-9][0-9]*}")
	@Consumes("application/json")
	public Response update(@PathParam("id") int id, KsSwitch entity) {
		if (entity == null) {
