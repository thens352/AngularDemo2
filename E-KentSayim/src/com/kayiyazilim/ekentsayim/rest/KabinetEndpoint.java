...
public class KabinetEndpoint {
	...
	public Kabinet create(Kabinet entity) {
	...
	public Response searchBarkod(@PathParam("barkod") Integer barkod) {
		Kabinet entity = dao.searchRest(barkod, null);
		if (entity != null)
	...
	public Response searchSeriNo(@PathParam("cihazSeriNo") String cihazSeriNo) {
		Kabinet entity = dao.searchRest(0, cihazSeriNo);
		if (entity != null)
			return Response.ok(entity).build();
		return Response.noContent().build();
	}
	...
	@GET
	@Produces("application/json")
	public List<Kabinet> listAll() {
		List<Kabinet> results = dao.findAll();
		return results;
	}
	...
	@Path("/{id:[0-9][0-9]*}")
	@Consumes("application/json")
	public Response update(@PathParam("id") int id, Kabinet entity) {
		if (entity == null) {
