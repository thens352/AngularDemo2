...
public class UpsresimEndpoint {
	...
	public Response searchBarkod(@PathParam("barkod") Integer barkod) {
		UPSResim entity = dao.searchRest(barkod, null);
		if (entity != null)
	...
	public Response searchSeriNo(@PathParam("cihazSeriNo") String cihazSeriNo) {
		UPSResim entity = dao.searchRest(0, cihazSeriNo);
		if (entity != null)
			return Response.ok(entity).build();
		return Response.noContent().build();
	}
	...
	@Produces("application/json")
	public UPSResim create(UPSResim entity) {
	...
	@GET
	@Produces("application/json")
	public List<UPSResim> listAll() {
		List<UPSResim> results = dao.findAll();
		return results;
	}
	...
	@Path("/{id:[0-9][0-9]*}")
	@Consumes("application/json")
	public Response update(@PathParam("id") int id, UPSResim entity) {
		if (entity == null) {
