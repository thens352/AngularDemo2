...
public class PdksEndpoint
{
   ...
   @EJB
	...
	public Response searchBarkod(@PathParam("barkod") Integer barkod) {
		PDKS entity = dao.searchRest(barkod, null);
		if (entity != null)
	...
	public Response searchSeriNo(@PathParam("cihazSeriNo") String cihazSeriNo) {
		PDKS entity = dao.searchRest(0, cihazSeriNo);
		if (entity != null)
			return Response.ok(entity).build();
		return Response.noContent().build();
	}
   ...
   @Produces("application/json")
   public PDKS create(PDKS entity)
   {
   ...
   @GET
   @Produces("application/json")
   public List<PDKS> listAll()
   {
      List<PDKS> results = dao.findAll();
      return results;
   }
   ...
   @Path("/{id:[0-9][0-9]*}")
   @Consumes("application/json")
   public Response update(@PathParam("id") int id, PDKS entity)
   {
      if (entity == null)
      {
