...
@Stateless
@Path("/ksnvr")
public class KsNVREndpoint {
	@EJB
	private KsNVRDao dao;
	@EJB
	...
	public Response searchBarkod(@PathParam("barkod") Integer barkod) {
		KsNVR entity = dao.searchRest(barkod, null);
		if (entity != null)
	...
	public Response searchSeriNo(@PathParam("cihazSeriNo") String cihazSeriNo) {
		KsNVR entity = dao.searchRest(0, cihazSeriNo);
		if (entity != null)
			return Response.ok(entity).build();
		return Response.noContent().build();
	}
	...
	@Produces("application/json")
	public KsNVR create(KsNVR entity) {
	...
	@GET
	@Produces("application/json")
	public List<KsNVR> listAll() {
		List<KsNVR> results = dao.findAll();
		return results;
	}
	...
	@Path("/{id:[0-9][0-9]*}")
	@Consumes("application/json")
	public Response update(@PathParam("id") int id, KsNVR entity) {
		if (entity == null) {
