...
public class NcSwitchEndpoint {
	...
	public Response searchBarkod(@PathParam("barkod") Integer barkod) {
		NcSwitch entity = dao.searchRest(barkod, null);
		if (entity != null)
	...
	public Response searchSeriNo(@PathParam("cihazSeriNo") String cihazSeriNo) {
		NcSwitch entity = dao.searchRest(0, cihazSeriNo);
		if (entity != null)
			return Response.ok(entity).build();
		return Response.noContent().build();
	}
	...
	@Produces("application/json")
	public NcSwitch create(NcSwitch entity) {
	...
	@GET
	@Produces("application/json")
	public List<NcSwitch> listAll() {
		List<NcSwitch> results = dao.findAll();
		return results;
	}
	...
	@Path("/{id:[0-9][0-9]*}")
	@Consumes("application/json")
	public Response update(@PathParam("id") int id, NcSwitch entity) {
			if (entity == null) {
