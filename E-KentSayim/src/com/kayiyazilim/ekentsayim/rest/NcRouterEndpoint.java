...
public class NcRouterEndpoint {
	...
	public Response searchBarkod(@PathParam("barkod") Integer barkod) {
		NcRouter entity = dao.searchRest(barkod, null);
		if (entity != null)
	...
	public Response searchSeriNo(@PathParam("cihazSeriNo") String cihazSeriNo) {
		NcRouter entity = dao.searchRest(0, cihazSeriNo);
		if (entity != null)
			return Response.ok(entity).build();
		return Response.noContent().build();
	}
	...
	@Produces("application/json")
	public NcRouter create(NcRouter entity) {
	...
	@GET
	@Produces("application/json")
	public List<NcRouter> listAll() {
		List<NcRouter> results = dao.findAll();
		return results;
	}
	...
	@Path("/{id:[0-9][0-9]*}")
	@Consumes("application/json")
	public Response update(@PathParam("id") int id, NcRouter entity) {
			if (entity == null) {
