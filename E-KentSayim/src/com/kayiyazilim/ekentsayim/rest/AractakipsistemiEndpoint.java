 ...
 */
@Stateless
@Path("/aractakipsistemi")
public class AractakipsistemiEndpoint {
	@EJB
	private AracTakipSistemiDao dao;
	@EJB
	...
	public Response searchBarkod(@PathParam("barkod") Integer barkod) {
		AracTakipSistemi entity = dao.searchRest(barkod, null);
		if (entity != null)
	...
	public Response searchSeriNo(@PathParam("cihazSeriNo") String cihazSeriNo) {
		AracTakipSistemi entity = dao.searchRest(0, cihazSeriNo);
		if (entity != null)
			return Response.ok(entity).build();
		return Response.noContent().build();
	}
	...
	public AracTakipSistemi create(AracTakipSistemi entity) {
	...
	public Response deleteById(@PathParam("id") int id) {
		AracTakipSistemi entity = dao.find(id);
		if (entity == null) {
	...
	@GET
	@Produces("application/json")
	public List<AracTakipSistemi> listAll() {
		List<AracTakipSistemi> results = dao.findAll();
		return results;
	}
	...
	@Path("/{id:[0-9][0-9]*}")
	@Consumes("application/json")
	public Response update(@PathParam("id") int id, AracTakipSistemi entity) {
		if (entity == null) {
