...
public class KioskEndpoint {
	...
	public Response searchBarkod(@PathParam("barkod") Integer barkod) {
		Kiosk entity = dao.searchRest(barkod, null);
		if (entity != null)
	...
	public Response searchSeriNo(@PathParam("cihazSeriNo") String cihazSeriNo) {
		Kiosk entity = dao.searchRest(0, cihazSeriNo);
		if (entity != null)
			return Response.ok(entity).build();
		return Response.noContent().build();
	}
	...
	@Produces("application/json")
	public Kiosk create(Kiosk entity) {
	...
	@GET
	@Produces("application/json")
	public List<Kiosk> listAll() {
		List<Kiosk> results = dao.findAll();
		return results;
	}
	...
	@Path("/{id:[0-9][0-9]*}")
	@Consumes("application/json")
	public Response update(@PathParam("id") int id, Kiosk entity) {
		if (entity == null) {
