 ...
 */
@Stateless
@Path("/printerscannerkartyazicisi")
public class PrinterScannerKartyazicisiEndpoint
{
   @EJB
   ...
   @EJB
	...
	public Response searchBarkod(@PathParam("barkod") Integer barkod) {
		Printer_Scanner_KartYazicisi entity = dao.searchRest(barkod, null);
		if (entity != null)
	...
	public Response searchSeriNo(@PathParam("cihazSeriNo") String cihazSeriNo) {
		Printer_Scanner_KartYazicisi entity = dao.searchRest(0, cihazSeriNo);
		if (entity != null)
			return Response.ok(entity).build();
		return Response.noContent().build();
	}
   ...
   @Produces("application/json")
   public Printer_Scanner_KartYazicisi create(Printer_Scanner_KartYazicisi entity)
   {
   ...
   {
      Printer_Scanner_KartYazicisi entity = dao.find(id);
      if (entity == null)
      {
   ...
   {
      Printer_Scanner_KartYazicisi entity;
      try
      {
         entity = dao.find(id);
      }
   ...
   @GET
   @Produces("application/json")
   public List<Printer_Scanner_KartYazicisi> listAll()
   {
      List<Printer_Scanner_KartYazicisi> results = dao.findAll();
      return results;
   }
   ...
   @Path("/{id:[0-9][0-9]*}")
   @Consumes("application/json")
   public Response update(@PathParam("id") int id, Printer_Scanner_KartYazicisi entity)
   {
      if (entity == null)
      {
