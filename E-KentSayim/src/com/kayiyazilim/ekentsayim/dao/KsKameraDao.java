...
@Stateless
public class KsKameraDao extends DAO<KsKamera> {
	...
	public KsKameraDao() {
		super(KsKamera.class);

	}
	
	public KsKamera searchRest(int barkod, String cihazSeriNo) {
		try {
			if(barkod!=0 || cihazSeriNo != null && !cihazSeriNo.equals("")){
			String sql = "select a from KsKamera as a where 1=1";
			if (barkod!=0)
			...
			return (KsKamera) query.getSingleResult();}
		} catch (Exception e) {
	...
	public List<KsKamera> search(KsKameraSearch search) {
		String sql = "select a from KsKamera as a " + " where 1=1";
