...
@Stateless
public class KsNVRDao extends DAO<KsNVR> {
	...
	public KsNVR searchRest(int barkod, String cihazSeriNo) {
		try {
			if(barkod!=0 || cihazSeriNo != null && !cihazSeriNo.equals("")){
			String sql = "select a from KsNVR as a where 1=1";
			if (barkod!=0)
			...
			return (KsNVR) query.getSingleResult();}
		} catch (Exception e) {
	...
	public List<KsNVR> search(KsNVRSearch search) {
		String sql = "select a from KsNVR as a " + " where 1=1";
