...
import com.kayiyazilim.ekentsayim.model.search.PDKSSearch;
	...
	@Stateless
	public class PDKSDao extends DAO<PDKS> {
		...
		public PDKS searchRest(int barkod, String cihazSeriNo) {
			try {
				if(barkod!=0 || cihazSeriNo != null && !cihazSeriNo.equals("")){
				String sql = "select a from PDKS as a where 1=1";
				if (barkod!=0)
				...
				return (PDKS) query.getSingleResult();}
			} catch (Exception e) {
		...
		public List<PDKS> search(PDKSSearch search) {
			String sql = "select a from PDKS as a " + " where 1=1 ";
