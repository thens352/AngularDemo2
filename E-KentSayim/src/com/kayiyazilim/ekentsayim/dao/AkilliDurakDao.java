...
@Stateless
public class AkilliDurakDao extends DAO<AkilliDurak> {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1964453416155383506L;

	/**
	 * 
	 */

	public AkilliDurakDao() {
		super(AkilliDurak.class);

	}
	
	public AkilliDurak searchRest(int barkod, String cihazSeriNo) {
		try {
			if(barkod!=0 || cihazSeriNo != null && !cihazSeriNo.equals("")){
			String sql = "select a from AkilliDurak as a where 1=1";
			if (barkod!=0)
			...
			return (AkilliDurak) query.getSingleResult();}
		} catch (Exception e) {
	...
	public List<AkilliDurak> search(AkilliDurakSearch search) {
		String sql = "select a from AkilliDurak as a " + " where 1=1";
