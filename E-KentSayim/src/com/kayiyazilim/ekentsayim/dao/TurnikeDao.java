...
@Stateless
public class TurnikeDao extends DAO<Turnike> {

	/**
	 * 
	 */
	private static final long serialVersionUID = 3312270327876415920L;
	...
	public Turnike searchRest(int barkod, String cihazSeriNo) {
		try {
			if(barkod!=0 || cihazSeriNo != null && !cihazSeriNo.equals("")){
			String sql = "select a from Turnike as a where 1=1";
			if (barkod!=0)
			...
			return (Turnike) query.getSingleResult();}
		} catch (Exception e) {
	...
	public List<Turnike> search(TurnikeSearch search) {
		String sql = "select a from Turnike as a " + " where 1=1";
