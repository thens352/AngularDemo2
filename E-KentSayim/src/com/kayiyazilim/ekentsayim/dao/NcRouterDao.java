...
@Stateless
public class NcRouterDao extends DAO<NcRouter> {
	...
	public NcRouterDao() {
		super(NcRouter.class);

	}

	public NcRouter searchRest(int barkod, String cihazSeriNo) {
		try {
			if (barkod != 0 || cihazSeriNo != null && !cihazSeriNo.equals("")) {
				String sql = "select a from NcRouter as a where 1=1";
				if (barkod != 0)
				...
				return (NcRouter) query.getSingleResult();
			}
		} catch (Exception e) {
	...
	public List<NcRouter> search(NcRouterSearch search) {
		String sql = "select a from NcRouter as a " + " where 1=1 ";
