...
@Stateless
public class AracTakipSistemiDao extends DAO<AracTakipSistemi> {

	/**
	 * 
	 */
	private static final long serialVersionUID = 4658890245846600707L;

	/**
	 * 
	 */

	public AracTakipSistemiDao() {
		super(AracTakipSistemi.class);

	}
	
	public AracTakipSistemi searchRest(int barkod, String cihazSeriNo) {
		try {
			if(barkod!=0 || cihazSeriNo != null && !cihazSeriNo.equals("")){
			String sql = "select a from AracTakipSistemi as a where 1=1";
			if (barkod!=0)
			...
			return (AracTakipSistemi) query.getSingleResult();}
		} catch (Exception e) {
	...
	public List<AracTakipSistemi> search(AracTakipSistemiSearch search) {
		...
		if (search.getHizmetVerenFirma() != null
				&& !"".equals(search.getHizmetVerenFirma())) {
			sql += " and LOWER(a.hizmetVerenFirma) like :hizmetVerenFirma";
		}
		...
		if (search.getHizmetVerenFirma() != null
				&& !search.getHizmetVerenFirma().equals("")) {
			query.setParameter("hizmetVerenFirma", "%"
					+ search.getHizmetVerenFirma().toLowerCase() + "%");
		}
