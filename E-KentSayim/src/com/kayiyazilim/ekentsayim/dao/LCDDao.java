...
@Stateless
public class LCDDao extends DAO<LCD> {
	...
	public LCD searchRest(int barkod, String cihazSeriNo) {
		try {
			if(barkod!=0 || cihazSeriNo != null && !cihazSeriNo.equals("")){
			String sql = "select a from LCD as a where 1=1";
			if (barkod!=0)
			...
			return (LCD) query.getSingleResult();}
		} catch (Exception e) {
	...
	public List<LCD> search(LCDSearch search) {
		String sql = "select a from LCD as a " + " where 1=1 ";
