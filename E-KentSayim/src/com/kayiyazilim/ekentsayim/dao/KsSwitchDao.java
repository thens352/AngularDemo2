...
@Stateless
public class KsSwitchDao extends DAO<KsSwitch> {

	private static final long serialVersionUID = 6503608119735444873L;

	public KsSwitchDao() {
		super(KsSwitch.class);

	}

	public KsSwitch searchRest(int barkod, String cihazSeriNo) {
		try {
			if (barkod != 0 || cihazSeriNo != null && cihazSeriNo.equals("")) {
				String sql = "select a from KsSwitch as a where 1=1";
				if (barkod != 0)
				...
				return (KsSwitch) query.getSingleResult();
			}
		} catch (Exception e) {
	...
	public List<KsSwitch> search(KsSwitchSearch search) {
		String sql = "select a from KsSwitch as a " + " where 1=1";
