...
@Stateless
public class NcSwitchDao extends DAO<NcSwitch> {
	...
	public NcSwitchDao() {
		super(NcSwitch.class);

	}

	public NcSwitch searchRest(int barkod, String cihazSeriNo) {
		try {
			if (barkod != 0 || cihazSeriNo != null && !cihazSeriNo.equals("")) {
				String sql = "select a from NcSwitch as a where 1=1";
				if (barkod != 0)
				...
				return (NcSwitch) query.getSingleResult();
			}
		} catch (Exception e) {
	...
	public List<NcSwitch> search(NcSwitchSearch search) {
		String sql = "select a from NcSwitch as a " + " where 1=1 ";
